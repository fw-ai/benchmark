...
- `--num-documents`: number of documents per rerank request. When set, each request is built with exactly this many documents (paragraphs). Requires `--tokenizer`. Uses `--dataset` (default `limericks`; or `code`) as the paragraph source. Use this to benchmark latency/throughput vs batch size.
- `--tokens-per-document`: for rerank with `--num-documents`, the target token count per document. When set, each document is built to be exactly this many tokens (by concatenating or truncating paragraphs from the dataset).
