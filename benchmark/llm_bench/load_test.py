# Do not import transformers at top level: under Locust, gevent is already active
# and transformers -> huggingface_hub -> trio raises. Rerank path uses the
# standalone "tokenizers" library when given a local path.
...
def _load_tokenizer_local(path: str):
    """Load tokenizer from a local path using the standalone tokenizers lib (no transformers/hub/trio).
    Use this under Locust to avoid gevent/trio conflict. path must be a directory containing
    tokenizer.json, or a path to tokenizer.json. Run download_tokenizer.py to create it.
    """
    ...
    if os.path.isfile(path) and path.endswith(".json"):
        ...
        if not os.path.isfile(json_path):
            raise FileNotFoundError(
                f"Local tokenizer path {path!r} has no tokenizer.json. "
                "Run: python download_tokenizer.py <model_id>"
            )
    else:
        raise ValueError(
            f"Tokenizer path {path!r} is not a local directory or tokenizer.json file. "
            "Under Locust use a local path (run download_tokenizer.py) to avoid gevent/trio conflict."
        )
...
class TranslationDataset:
    def __init__(
        ...
        import transformers  # lazy: avoid loading under Locust (gevent/trio conflict)
...
class RerankNumDocumentsDataset:
    """Dataset for rerank that yields prompts with exactly num_documents paragraphs.

    Each prompt is built by sampling paragraphs from a source file (e.g. limericks)
    so that prompt.split('\n\n') has exactly num_documents elements. When
    tokens_per_document is set, each document is built to be exactly that many tokens
    (by concatenating/truncating paragraphs). Used when --rerank and --num-documents
    are set. Uses the standalone tokenizers library (local path only) to avoid
    transformers/huggingface_hub/trio under gevent.
    """
    ...
    def __init__(
        ...
        num_documents: int,
        tokenizer_path: str,
        tokens_per_document: Optional[int] = None,
    ):
        self._tokenizer = _load_tokenizer_local(tokenizer_path)
        self._num_documents = num_documents
        self._tokens_per_document = tokens_per_document
        with open(path, "r") as f:
            text = f.read()
        self._paragraphs = [p.strip() for p in text.split("\n\n") if p.strip()]
        if not self._paragraphs:
            raise ValueError(f"No paragraphs found in {path} (split by '\\n\\n')")
    ...
    def _make_document(self, target_tokens: int) -> str:
        """Build a single document that tokenizes to exactly target_tokens tokens."""
        parts = []
        while True:
            p = self._paragraphs[
                random.randint(0, len(self._paragraphs) - 1)
            ]
            parts.append(p)
            text = "\n\n".join(parts)
            if self._token_count(text) >= target_tokens:
                break
        full_text = "\n\n".join(parts)
    ...
    def __next__(self):
        if self._tokens_per_document is not None:
            # Each document is exactly tokens_per_document tokens
            documents = [
                self._make_document(self._tokens_per_document)
                for _ in range(self._num_documents)
            ]
            prompt = "\n\n".join(documents)
        else:
            # Each document is one sampled paragraph (variable length)
            indices = [
                random.randint(0, len(self._paragraphs) - 1)
                for _ in range(self._num_documents)
            ]
            prompt = "\n\n".join(self._paragraphs[i] for i in indices)
...
class DatasetHolder:
    ...
    def _create_dataset(cls, options: argparse.Namespace):
        num_documents = getattr(options, "num_documents", None)
        if getattr(options, "rerank", False) and num_documents is not None:
            assert options.tokenizer is not None, (
                "--tokenizer is required for rerank with --num-documents"
            )
            assert options.dataset in (
                "limericks",
                "code",
            ), (
                "--num-documents requires --dataset limericks or code (not JSONL)"
            )
            dataset_file = "limericks.txt" if options.dataset == "limericks" else "code.txt"
            path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), dataset_file
            )
            tokens_per_document = getattr(
                options, "tokens_per_document", None
            )
            return RerankNumDocumentsDataset(
                path=path,
...
class LLMUser(HttpUser):
    ...
    def _on_start(self):
        ...
        # Include num_documents when using rerank so summary CSV identifies batch size per row
        num_docs = getattr(self.environment.parsed_options, "num_documents", None)
        logging_params["num_documents"] = num_docs if num_docs is not None else ""
        tokens_per_doc = getattr(
            self.environment.parsed_options, "tokens_per_document", None
        )
        logging_params["tokens_per_document"] = (
            tokens_per_doc if tokens_per_doc is not None else ""
        )
...
def init_parser(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="For rerank: number of documents per request. When set, prompts are generated with exactly this many documents. Requires --tokenizer and --dataset limericks or code.",
    )
    parser.add_argument(
        "--tokens-per-document",
        type=int,
        default=None,
        help="For rerank with --num-documents: target token count per document. When set, each document is built to be exactly this many tokens (by concatenating/truncating paragraphs from the dataset).",
    )
    parser.add_argument(
        "--return-logits",
        type=int,
