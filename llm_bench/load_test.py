...
class DatasetHolder:
    ...
    def _create_dataset(cls, options: argparse.Namespace):
        if options.dataset.startswith("@"):
            ...
            assert options.tokenizer is not None, "--tokenizer is required for limerics or code dataset"
            if options.dataset == "limerics":
                if options.prompt is None:
                    prompt = (
                        "Translate the limericks above to Spanish, then re-write limericks using 10 different styles."
                    )
                else:
    ...
    def parse_output_json(self, data):
        ...
        if self.parsed_options.chat:
            if self.parsed_options.stream:
                text = (choice["delta"].get("reasoning_content", "") or "") + (
