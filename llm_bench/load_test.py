...
class LLMUser(HttpUser):
    ...
    def _do_generate_text(self):
        ...
        with self.client.post(
            ...
            stream=True,
            catch_response=True,
            timeout=getattr(self.environment.parsed_options, "request_timeout", 60),
        ) as response:
            combined_text = ""
            done = False
            completion_tokens = None
...
def init_parser(parser):
    ...
    parser.add_argument(
        ...
        type=float,
        default=60,
        help="Per-request HTTP client timeout in seconds. Increase for long-context prefill workloads.",
    )
